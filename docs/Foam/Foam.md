...
- [Foam](#foam)
  - [基础配置与上手](#基础配置与上手)
    ...
    - [配置笔记库](#配置笔记库)
      - [在 GitHub 上建立笔记库](#在-github-上建立笔记库)
      - [在本地使用 Foam](#在本地使用-foam)
  - [初步上手](#初步上手)
    - [创建新的笔记](#创建新的笔记)
    - [创建双向链接](#创建双向链接)
    - [笔记元数据](#笔记元数据)
    ...
    - [侧边栏面板](#侧边栏面板)
  - [生产力从美化开始](#生产力从美化开始)
  - [使用代码片段和模板来提高效率](#使用代码片段和模板来提高效率)
    - [代码片段的配置介绍](#代码片段的配置介绍)
    - [创建笔记元数据](#创建笔记元数据)
    - [插入时间戳](#插入时间戳)
    - [创建和使用模板](#创建和使用模板)
    - [使用 note-macros 扩展（不推荐）](#使用-note-macros-扩展不推荐)
  - [从其它源获取内容](#从其它源获取内容)
    - [从网页捕获内容](#从网页捕获内容)
    - [从 iOS 端输入内容](#从-ios-端输入内容)
  - [并不完美但未来可期](#并不完美但未来可期)

> [转自少数派](https://sspai.com/post/70956)
...
VS Code 安装成功并启动后，首先要要做的是点击侧边栏扩展图标（需要注意 VSCodium 开箱配置的扩展源与 VS Code 并不一致，可以参考 这篇文章 更改为 VS Code 的扩展源），搜索并安装安装简体中文语言包和自己喜欢的颜色主题。
...
![X](../../attachments/2022-07-18-12-58-17.png)
...
![X](../../attachments/2022-07-18-12-59-38.png)

## 生产力从美化开始

正如前面所说，赏心悦目的界面也会对情绪和效率产生积极影响，因此对 Foam 的编辑器和笔记预览界面进行美化也是非常重要的一环。

Foam 使用 Markdown 作为笔记的文件格式，一方面其通用性便利了跨平台使用，另一方面也可以利用 CSS 文件对其进行美化。
...
![X](../../attachments/2022-07-18-13-01-02.png)
...
![X](../../attachments/2022-07-18-13-05-39.png)
...
也可以在 Foam 中定义其他模板，如 Todo List、Weekly Note 等$^1$，都会极大地便利个人的日常学习和各项工作。
